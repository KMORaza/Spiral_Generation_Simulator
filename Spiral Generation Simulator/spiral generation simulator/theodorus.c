...
void draw_theodorus(cairo_t *cr, AppData *app_data) {
    if (!cr || !app_data) {
        g_print("Error: Null Cairo context or data in draw_theodorus\n");
        return;
    }
    int width = gtk_widget_get_width(app_data->drawing_area);
    int height = gtk_widget_get_height(app_data->drawing_area);
    g_print("Drawing Theodorus with size %d x %d, point %d/%d\n",
    ...
    for (int i = 0; i < app_data->spiral_data.current_point - 1 && i < app_data->spiral_data.max_points - 2; i++) {
        cairo_move_to(cr, 0, 0);
        cairo_line_to(cr, app_data->spiral_data.x[i], app_data->spiral_data.y[i]);
        cairo_line_to(cr, app_data->spiral_data.x[i+1], app_data->spiral_data.y[i+1]);
        cairo_close_path(cr);
        cairo_stroke(cr);
    }
