...
void draw_lissajous(cairo_t *cr, AppData *app_data) {
    if (!cr || !app_data) {
        g_print("Error: Null Cairo context or data in draw_lissajous\n");
        return;
    }
    int width = gtk_widget_get_width(app_data->drawing_area);
    int height = gtk_widget_get_height(app_data->drawing_area);
    g_print("Drawing Lissajous Spiral with size %d x %d, point %d/%d\n",
    ...
    if (app_data->spiral_data.current_point > 50) {
        ...
        cairo_show_text(cr, "Lissajous Spiral");
    }
}
