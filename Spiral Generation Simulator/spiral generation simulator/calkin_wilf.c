...
void draw_calkin_wilf(cairo_t *cr, AppData *app_data) {
    if (!cr || !app_data) {
        g_print("Error: Null Cairo context or data in draw_calkin_wilf\n");
        return;
    }
    int width = gtk_widget_get_width(app_data->drawing_area);
    int height = gtk_widget_get_height(app_data->drawing_area);
    g_print("Drawing Calkin-Wilf with size %d x %d, point %d/%d\n",
    ...
    for (int i = 0; i < app_data->spiral_data.current_point && i < app_data->spiral_data.max_points; i++) {
        cairo_arc(cr, app_data->spiral_data.x[i], app_data->spiral_data.y[i], 2.0, 0, 2 * M_PI);
        cairo_fill(cr);
    }
}
