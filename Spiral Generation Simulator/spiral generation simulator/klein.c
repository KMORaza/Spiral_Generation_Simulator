...
void draw_klein(cairo_t *cr, AppData *app_data) {
    ...
    for (int i = 0; i < app_data->spiral_data.current_point && i < app_data->spiral_data.max_points - 1; i++) {
        ...
        if (i % 100 == 0) {
            g_print("Point %d: x=%.2f, y=%.2f\n", i, app_data->spiral_data.x[i], app_data->spiral_data.y[i]);
        }
    }
    cairo_stroke(cr);
    if (app_data->spiral_data.current_point > 50) {
        ...
        cairo_show_text(cr, "Klein Spiral (r = a θ / sin(θ))");
    }
}
